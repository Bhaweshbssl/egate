                ...
                <tr>
                  ...
                  @foreach($products as $product)
                    @if($product->category->status==1)  
                      @foreach($productsattr as $attr)
                        @if($product->id == $attr->products_id)
                          <td style="text-align: left;"><a href="{{url('/product-detail',$product->id)}}">CLICK HERE</a></td>
                        @endif
                      @endforeach
                    @endif
                  @endforeach  
            ...
            @if($product->category->status==1)
              @if($count<=1)
                @foreach($categories as $category)
                  ...
                  <?php
                    $sub_categories=DB::table('categories')->orderby('id','asc')->select('id','name')->where([['parent_id',$category->id],['status',1]])->get();
                  ?>
